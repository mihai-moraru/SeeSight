...
private enum Language {
    ...
    static private func applyMultimodalRotaryPositionEmbedding(
        _ q: MLXArray, _ k: MLXArray, cos: MLXArray, sin: MLXArray, mropeSection: [Int]
    ) -> (MLXArray, MLXArray) {
        var cos = cos
        var sin = sin
        cos = concatenated(
            [
                cos[0..., 0 ..< mropeSection[0]],
                cos[1..., mropeSection[0] ..< (mropeSection[0] + mropeSection[1])],
                cos[2..., (mropeSection[0] + mropeSection[1])...],
            ],
            axis: -1
        )[0..., .newAxis, 0..., 0...]
        ...
        sin = concatenated(
            [
                sin[0..., 0 ..< mropeSection[0]],
                sin[1..., mropeSection[0] ..< (mropeSection[0] + mropeSection[1])],
                sin[2..., (mropeSection[0] + mropeSection[1])...],
            ],
            axis: -1
        )[0..., .newAxis, 0..., 0...]
    ...
    fileprivate class Attention: Module {
        ...
        public init(_ args: FastVLMConfiguration.TextConfiguration) {
            ...
            if let section = args.ropeScaling?["mrope_section"]?.ints {
                self.mropeSection = section.map { $0 * 2 }
            } else {
                self.mropeSection = [2 * 2, 1 * 2, 1 * 2]
            }
        ...
        public func callAsFunction(
            ...
            let mropeIndices = MLXArray(0 ..< 3).reshaped(3, 1) * ones([1, L], type: Int32.self)
                + MLXArray(Int32(offset))
            let (cos, sin) = rope(mropeIndices, offset: 0)
            (queries, keys) = Language.applyMultimodalRotaryPositionEmbedding(
                queries, keys, cos: cos, sin: sin, mropeSection: mropeSection)
    ...
    fileprivate class MLP: Module, UnaryLayer {
        ...
        public init(_ dim: Int, hiddenDim: Int) {
    ...
    fileprivate class FastVLMDecoderLayer: Module {
        ...
        public init(_ args: FastVLMConfiguration.TextConfiguration) {
            ...
            _mlp.wrappedValue = MLP(args.hiddenSize, hiddenDim: args.intermediateSize)
...
public class FastVLM: Module, VLMModel, KVCacheDimensionProvider {
    ...
    static public var modelConfiguration: ModelConfiguration {
        ...
        if let url = bundle.url(forResource: "config", withExtension: "json", subdirectory: "model") {
            return ModelConfiguration(directory: url.deletingLastPathComponent())
        }
        // Fallback to looking for model directory
        if let url = bundle.url(forResource: "model", withExtension: nil) {
            return ModelConfiguration(directory: url)
        }
        fatalError("Model not found in bundle. Please download the model using get_pretrained_mlx_model.sh")
    }
    ...
    static public func register(modelFactory: VLMModelFactory) {
        ...
        modelFactory.processorRegistry.registerProcessorType("FastVLMProcessor") {
            url, tokenizer in
            let configuration = try JSONDecoder().decode(
                FastVLMPreProcessorConfiguration.self, from: Data(contentsOf: url))
            return FastVLMProcessor(configuration, tokenizer: tokenizer)
        ...
        -> MLXArray
    {
        ...
        var imageFeatures = visionModel(pixelValues, gridThw: gridThw)
